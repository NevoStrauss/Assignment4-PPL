...
describe('2.1 (PromisedStore)', () => {
    ...
    it('getAll retrieves an array', async () => {
        ...
        await expect(getAll(store, ['a','b','c'])).to.be.rejectedWith(MISSING_KEY)

    })
})

describe('2.2 (asyncMemo)', () => {
    it('memorizes calls', async () => {
        let ret = 'cached'
        const memo = asyncMemo((x) => ret)
...
describe('2.4 (asyncWaterfallWithRetry)', () => {
    it('executes sequence', async () => {
        const v = await asyncWaterfallWithRetry([async () => 1, async (v: any) => v + 1, async (v: any) => v * 2 ])
        expect(v).to.equal(4)
    })

    it('retries twice', async () => {
        let attempt = 1
        const v = await asyncWaterfallWithRetry([async () => 1, async (v: any) => {
            if (attempt == 3)
                return v + 1
            attempt += 1
            throw Error()
